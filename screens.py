...
class DashboardScreen(BaseScreen):
    ...
    def _show_profile(self):
        role = self.app.current_user_role or 'member'
        if role == 'member':
            self.app.navigate_to('my_profile')
        else:
...
class MemberProfileScreen(BaseScreen):
    ...
    def _render(self, member, accounts, loans):
        ...
        action_row = MDBoxLayout(size_hint_y=None, height=dp(52), spacing=dp(8))
        for text, color, screen in [
            ('Deposit', 'success', 'deposit'),
            ('Withdraw', 'error', 'withdrawal'),
            ('Loan', 'quaternary', 'loan_application'),
            ('Repay', 'quinary', 'repayment'),
        ]:
            btn = MDRaisedButton(
                text=text,
                md_bg_color=get_color(color),
                size_hint_x=1,
                on_release=lambda x, s=screen, mid=member['id']: self.app.navigate_to(s, member_id=mid)
            )
...
class NavigationDrawerContent(MDBoxLayout):
    ...
    def rebuild_for_role(self, role):
        self._build_menu(role or 'member')
